...
# ---------------------------------------------------------------------------
# CELL 2 — Products (10 rows — Enercare public rate card)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CELL 4 — Service Accounts (54 rows)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CELL 5 — Equipment registry (inline from SQL seed, selected subset for Spark)
# ---------------------------------------------------------------------------
...
# ===========================================================================
# CELL ** — CALL CENTER EXTENSION
# ===========================================================================


# ---------------------------------------------------------------------------
# CELL CC-1 — cc_agents (15 rows)
# ---------------------------------------------------------------------------
from pyspark.sql.types import *
from datetime import date
...
# ---------------------------------------------------------------------------
# CELL CC-2 — ref_cc_billing_adj_category (12 rows)
# ---------------------------------------------------------------------------
from pyspark.sql.types import *
...
# ---------------------------------------------------------------------------
# CELL CC-3 — fct_cc_interactions (300 rows with demo correlation)
...
CORR_DECLINES  = {1, 2, 3, 4, 5, 6}  # 6/14 → 57% acceptance
...
def _make_row(iid, cust_id, q, idate, pp_out, is_corr_billing):
    ...
    chan   = random.choices(["inbound_voice", "callback", "chat"], weights=[78, 14, 8])[0]
...
for idx, cust_id in enumerate(CORR_CUSTOMERS):
    bdate = date(2026, 1, 4) + timedelta(days=idx * 2)
...
for seq, q in enumerate(remaining_queues):
    ...
    if q == "pp_renewal":
        pp_out = random.choices(["declined", "accepted", "callback"], weights=[22, 68, 10])[0]
    else:
...
int_data = [row[:16] for row in interactions_raw]
...
# ---------------------------------------------------------------------------
# CELL CC-4 — fct_cc_transcript_turns (~12 turns/interaction)
...
AGENT_PHRASES = {"billing": BILLING_AGENT}
INTENT_DIST = {
